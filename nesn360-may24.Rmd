---
title: "NESN 360 Monthly Subscriber Report - May 2024"
output: 
...
df_s <- read_csv('subscriptions_2024_06_03.csv') #CHANGE HERE
roku <- read_csv('roku_subscriptions_2024_06_03.csv') %>% #CHANGE HERE
...
wk_no <- 22 # CHANGE HERE
wk_no_off <- 22  # CHANGE HERE
wk_range <- c(mdy('05/26/24'):mdy('06/01/24')) # CHANGE HERE
wk_end_epi <- mdy('06/01/24') # CHANGE HERE
...
net_subs_plot <- subs_2 %>% 
  ...
  labs(x = '',
       y = 'Net Subscribers',
       title = "May 2024 - Net NESN 360 Subscribers by day") + ###
...
active_subs_plot <- subs_2 %>% 
  filter(year(date) == 2024 & month(date) == 5) %>% ####3
  ...
  labs(x = '',
       ...
       title = "May 2024 - Active NESN 360 Subscribers by day") + #####
  theme(panel.grid.minor = element_blank(),
        plot.title.position = 'plot',
        plot.title = element_text(face = 'bold', size = 11))
...
```{r}
test_tab <- asdf %>% 
  filter(year == 2024 & month(wk_end) == 5) %>% ####
...
```{r}
cur_month <- df %>% 
  filter(year(sub_start_date) == 2024 & month(sub_start_date) == 5)###

prev_churn <- df %>% 
  filter(sub_start_date < mdy('5/1/2024')) %>% ####
  filter(status == 'canceled') %>% 
  select(cognito_id) %>% 
  as_vector()
