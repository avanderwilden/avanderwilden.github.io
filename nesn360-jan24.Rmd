...
wk_end_epi <- mdy('02/01/24') # CHANGE HERE
...
valueBox(value = scales::comma(net_num), icon = 'fa-user',
         caption = 'Net Subscribers for January', color = 'orange')
```
...
```{r}
cur_month <- df %>% 
  filter(year(sub_start_date) == 2024 & month(sub_start_date) == 1)

prev_churn <- df %>% 
  filter(sub_start_date < mdy('1/1/2024')) %>% 
  filter(status == 'canceled') %>% 
  select(cognito_id) %>% 
  as_vector()
