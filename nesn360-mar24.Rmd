---
title: "NESN 360 Monthly Subscriber Report - February 2024"
output: 
...
df_s <- read_csv('subscriptions_2024_04_01.csv') #CHANGE HERE
roku <- read_csv('roku_subscriptions_2024_04_01.csv') %>% #CHANGE HERE
...
wk_no <- 13 # CHANGE HERE
wk_no_off <- 13  # CHANGE HERE
wk_range <- c(mdy('03/31/24'):mdy('04/06/24')) # CHANGE HERE
wk_end_epi <- mdy('04/01/24') # CHANGE HERE
...
net_subs_plot <- subs_2 %>% 
  ...
  labs(x = '',
       y = 'Net Subscribers',
       title = "March 2024 - Net NESN 360 Subscribers by day") +
...
active_subs_plot <- subs_2 %>% 
  ...
  labs(x = '',
       ...
       title = "March 2024 - Active NESN 360 Subscribers by day") +
  theme(panel.grid.minor = element_blank(),
        plot.title.position = 'plot',
        plot.title = element_text(face = 'bold', size = 11))
...
```{r}
test_tab <- asdf %>% 
  filter(year == 2024 & month(wk_end) == 3) %>% 
...
```{r}
cur_month <- df %>% 
  filter(year(sub_start_date) == 2024 & month(sub_start_date) == 3)
